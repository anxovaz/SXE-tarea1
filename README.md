...
| Versión de PHP | 8.3 o superior | 8.3 o superior | Se usará 8.4.24, que es la última versión estable que se instala por defecto con apt en debian13| [Documentación oficial de wordpress](https://wordpress.org/about/requirements/) |
...
Guiándome de los tutoriales de la `Bibliografía` (al final de este `README`) empiezo instalando `apache` y `mariadb`.
...
Creo la `base de datos` y el usuario con el que `wordpress` accederá a ella.
...
Habilito el nuevo sitio (`anxo.com`) y deshabilito el `default`, después reinicio el servicio de `apache`:
...
Una vez con los nuevos repositorios procedí a ejecutar `apt update` y ahora sí que estaba listo para instalar los paquetes necesarios.

### Tutoriales

Al terminar con el tutorial de Voidnull no crea el archivo ‘/etc/apache2/sites-available/anxo.com.conf’ ni activa los sitios para que apache funcione, para ello me fijo en el tutorial de Debian Wiki.
...
- Gemini - Ayuda con repositorios apt
    ...
    Prompt: ¿Cuáles son los repositorios oficiales de debian 13 de apt?
